## Phosphor
A dark green terminal-inspired theme with yellow text and cyan accents.

## Color Palette
- **Background:** #082408
- **Text:** #FFE000
- **Accent:** #00E5FF

## Screenshot Examples

### Theme overall view
TypeScript file showing inline type errors and a color palette overview, as well as showing JSX markup with nested tags and string attributes.
![TypeScript Example](images/screenshot-01.png "Theme overall view")
![TypeScript nested tags and string attribute example](images/screenshot-05.png "Nested tags and string attributes example")

### Git diff view
Showing added/removed lines across a TypeScript class
![Git diff view](images/screenshot-02.png "Git diff view")

### Project-wide search view
Shows a project-wide search for "panic" across three TypeScript files
![Project-wide search view](images/screenshot-03.png "Project-wide search")

### Rust example
Rust file to show syntax highlighting across enums, structs, attributes, and doc comments
![Rust example](images/screenshot-04.png "Rust example")
